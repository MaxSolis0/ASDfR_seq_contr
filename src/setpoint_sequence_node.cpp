...
SetpointSequenceNode::SetpointSequenceNode(): Node("setpoint_sequence_node")
{
  ...
  this->declare_parameter("setpoints_array", std::vector<double>{}); // setpoint coordinates
  // Controller parameters (Proportional)
  this->declare_parameter("kp_linear", 10.0); //proportional controller gain for translation
  this->declare_parameter("kp_angular", 50.0); //proportional controller gain for rotation
  ...
  paramCallbackHandle_ = this->add_on_set_parameters_callback(
    std::bind(&SetpointSequenceNode::parametersCallback, this, std::placeholders::_1));
  ...
  targetPub_ = this->create_publisher<geometry_msgs::msg::Point>(
      "/target", 10);
  ...
  poseSub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
      "/output/robot_pose",
      10,
      std::bind(&SetpointSequenceNode::poseCallback, this, std::placeholders::_1));
  //Subscribe to get Point for object tracking
  objectSub_ = this->create_subscription<geometry_msgs::msg::Point>(
      "/tracked_CoM", 
      10,
      std::bind(&SetpointSequenceNode::objectController, this, std::placeholders::_1));
  //Subscribe to get Point for object tracking
...
SetpointSequenceNode::parametersCallback(const std::vector<rclcpp::Parameter> & params)
{
...
//parse pose
void SetpointSequenceNode::poseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
    currentX_ = msg->pose.position.x;
    currentY_ = msg->pose.position.y;
    currentTheta_ = msg->pose.orientation.z; //Pose should be quaternion, but only z is used for theta_z and the rest are static
}
...
void SetpointSequenceNode::parseView(const geometry_msgs::msg::PointStamped::SharedPtr msg)
{
    viewX_  = msg->point.x;
}

void SetpointSequenceNode::parseBbox(const vision_msgs::msg::BoundingBox2D msg)
{
    bboxWidth_  = msg.size_x;
}

void SetpointSequenceNode::objectController(const geometry_msgs::msg::Point::SharedPtr msg)
{
    ...
    //Image errors
    if (mode_){
      errorX = objectX - (width_/2.0); //orientation error for closed loop 1.2.3
    } else {
      errorX = objectX - viewX_; //orientation error for open loop 1.2.2
    }
    double errorSize = bboxWidth_ - desiredWidth_; //size error
    double v = kpLinear_/200.0 * errorSize; //smaller gain because of increased frequency
    double omega = kpAngular_/200.0 * errorX; //smaller gain because of increased frequency
