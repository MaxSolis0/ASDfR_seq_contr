 ...
 * @param setpoints [bool]: Which commands the robot should follow, true for
 * coordinate setpoints, false for object tracking. Default: true.
 * @param setpoints_array [vector[double]]: The coordinates of the setpoints in
 * format (duration, x, y, z). Default: {} (empty)
 * @param kp_linear [double]: Proportional controller gain for translation.
 * Default: 10.0
 * @param wheel_base [double]: The distance between wheels. Default: 0.209 (from
 * relbot_simulator.hpp)
 * @param mode [bool]: If the robot should assume input from camera (simulation
 * open loop) or moving_camera (simualtion closed loop). Default: true
 * @param width [int]: The width of the window of camera view. Default: 360
 * @param desired_width [double]: The desired width in pixels of the object
 * tracked to determine if the robot should move closer or farther from it.
 * Default: 100.0
 */
...
class SetpointSequenceNode : public rclcpp::Node {
 ...
 private:
  ...
  /**
   * @brief Parse the sequence recieved into array of Points and array of
   * durations
   *
   */
  void parseSequence();
   ...
   *
   * @param msg: Robot pose, orientation is quaternion format but actually
   * expresses only z as Euler angle while x,y,w are unchanged.
   */
   ...
   *
   * @param left: Left wheel speed, opposite sign of traditional Differential
   * Drive model
  ...
  /**
   * @brief Get x coordinate of simulated moving camera view for orientation
   * control
   *
   * @param msg: Pixel Coordinates of center from simulated moving camera in
   * reference to static camera.
   */
   ...
   *
   * @param msg: Coordinates and size of bounding box in reference to static
   * camera.
   */
  void parseBbox(const vision_msgs::msg::BoundingBox2D msg);

  /**
   * @brief Controller to track object in camera view with open loop static
   * camera mode and closed loop simulated moving camera mode
   *
